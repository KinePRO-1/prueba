...
function normalizeRoutineSpeechText(value) {
  return String(value || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-z0-9,.\s]/g, " ")
    .replace(/\bvara\b/g, "barra")
    .replace(/\barrodillada\b/g, "arrodillado")
    .replace(/\buna pierna\b/g, "unipodal")
    .replace(/\bun pie\b/g, "unipodal")
    .replace(/\s+/g, " ")
    .trim();
}
...
const VOICE_NUMBER_PATTERN = "\\d+(?:[,.]\\d+)?|un|una|uno|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez|once|doce|trece|catorce|quince|veinte|treinta|cuarenta|cincuenta|sesenta";

function getVoiceNumber(value) {
  const number = spanishNumberToValue(value);
  return Number.isFinite(number) ? number : null;
}

function compactRoutineText(value) {
  return normalizeRoutineSpeechText(value).replace(/\s+/g, "");
}

function getTokenSet(value) {
  return new Set(normalizeRoutineSpeechText(value).split(/\s+/).filter((token) => token.length > 2));
}
...
function scoreExerciseMatch(detectedName, exercise) {
  const query = normalizeRoutineSpeechText(detectedName);
  const candidate = normalizeRoutineSpeechText(`${exercise.name} ${exercise.category || ""} ${exercise.type || ""}`);
  const queryCompact = compactRoutineText(query);
  const candidateCompact = compactRoutineText(exercise.name);
  if (!query || !candidateCompact) return 0;
  if (query === normalizeRoutineSpeechText(exercise.name) || queryCompact === candidateCompact) return 1;
  if (candidate.includes(query) || candidateCompact.includes(queryCompact)) return 0.93;
  if (query.includes(normalizeRoutineSpeechText(exercise.name)) || queryCompact.includes(candidateCompact)) return 0.9;
  ...
  const tokenScore = shared / Math.max(1, Math.max(queryTokens.size, candidateTokens.size));
  const distance = levenshteinDistance(queryCompact, candidateCompact);
  const fuzzyScore = 1 - distance / Math.max(queryCompact.length, candidateCompact.length, 1);
  return Math.max(tokenScore * 0.86, fuzzyScore * 0.78);
}

function findVoiceExerciseMatches(detectedName) {
  return getCombinedExerciseLibrary()
    .map((exercise, index) => ({
      ...exercise,
      libraryIndex: index,
      score: scoreExerciseMatch(detectedName, exercise)
    }))
    .sort((a, b) => b.score - a.score)
    .slice(0, 3);
}

function splitRoutineVoiceSegments(text) {
  return String(text || "")
    .replace(/\b(?:y\s+despues|y\s+después|despues|después|luego|seguido de|posteriormente|tambien|también)\b/gi, ".")
    .split(/[\n.;]+/)
    .map((segment) => segment.trim())
    .filter(Boolean);
}

function parseRoutineVoiceSegment(segment) {
  const source = String(segment || "").replace(/\s+/g, " ").trim();
  const normalized = normalizeRoutineSpeechText(source);
  if (!normalized) return null;

  const xMatch = normalized.match(new RegExp(`\\b(${VOICE_NUMBER_PATTERN})\\s*(?:x|por)\\s*(${VOICE_NUMBER_PATTERN})\\b`, "i"));
  const setsMatch = normalized.match(new RegExp(`\\b(${VOICE_NUMBER_PATTERN})\\s*(?:series?|serie)\\b`, "i"));
  const seriesRepsMatch = normalized.match(new RegExp(`\\b(?:series?|serie)\\s+de\\s+(${VOICE_NUMBER_PATTERN})(?:\\s*(?:repeticiones?|reps?))?\\b`, "i"));
  const repsMatch = normalized.match(new RegExp(`\\b(${VOICE_NUMBER_PATTERN})\\s*(?:repeticiones?|reps?)\\b`, "i"));
  const secondsMatch = normalized.match(new RegExp(`\\b(${VOICE_NUMBER_PATTERN})\\s*(?:segundos?|seg)\\b`, "i"));
  const noLoad = /\bsin\s+carga\b/i.test(normalized);
  const loadMatch = normalized.match(new RegExp(`\\b(${VOICE_NUMBER_PATTERN})\\s*(?:kilos?|kg)\\b`, "i"));

  const sets = getVoiceNumber(xMatch?.[1]) || getVoiceNumber(setsMatch?.[1]) || 1;
  const seconds = getVoiceNumber(secondsMatch?.[1]) || "";
  const reps = seconds ? "" : (getVoiceNumber(xMatch?.[2]) || getVoiceNumber(seriesRepsMatch?.[1]) || getVoiceNumber(repsMatch?.[1]) || "");
  const loadValue = getVoiceNumber(loadMatch?.[1]);
  const load = noLoad ? "Sin carga" : loadValue ? formatKgLoad(loadValue) : "Sin carga";

  let detectedName = normalized
    .replace(new RegExp(`\\b(${VOICE_NUMBER_PATTERN})\\s*(?:x|por)\\s*(${VOICE_NUMBER_PATTERN})\\b`, "gi"), " ")
    .replace(new RegExp(`\\b(${VOICE_NUMBER_PATTERN})\\s*(?:series?|serie)(?:\\s+de\\s+(${VOICE_NUMBER_PATTERN})(?:\\s*(?:repeticiones?|reps?))?)?\\b`, "gi"), " ")
    .replace(new RegExp(`\\b(${VOICE_NUMBER_PATTERN})\\s*(?:repeticiones?|reps?|segundos?|seg|kilos?|kg)\\b`, "gi"), " ")
    .replace(/\bsin\s+carga\b/gi, " ")
    .replace(/\b(agrega|agregá|agregar|suma|sumar|carga|cargar|hacer|ejercicio|primero|despues|después|luego|seguido)\b/g, " ")
    .replace(/\b(con|de|del|la|el|y)\s*$/g, " ")
    .replace(/\s+/g, " ")
    .trim();

  if (!detectedName || (!reps && !seconds)) return null;
  const matches = findVoiceExerciseMatches(detectedName);
  return {
    id: uid("voice"),
    rawText: source,
    detectedName,
    matches,
    selectedLibraryIndex: matches[0]?.libraryIndex ?? "",
    sets,
    reps,
    seconds,
    load
...
function parseRoutineVoiceText(text) {
  return splitRoutineVoiceSegments(text).map(parseRoutineVoiceSegment).filter(Boolean);
}

function renderVoiceRoutineSuggestions() {
  const container = $("#voice-routine-suggestions");
  if (!container) return;
  container.innerHTML = routineVoiceSuggestions.map((suggestion) => {
    const selected = suggestion.matches.find((match) => String(match.libraryIndex) === String(suggestion.selectedLibraryIndex)) || suggestion.matches[0];
    const score = selected ? Math.round(selected.score * 100) : 0;
    return `
      <article class="voice-suggestion-card" data-voice-card="${suggestion.id}">
        <div class="voice-suggestion-head">
          <div>
            <span>Detectado</span>
            <strong>"${escapeHtml(suggestion.rawText)}"</strong>
            <small>Texto limpio: ${escapeHtml(suggestion.detectedName)}</small>
          </div>
          <span class="chip ${score >= 82 ? "stable" : score >= 62 ? "controlled" : "high"}">${score ? `${score}%` : "revisar"}</span>
        </div>
        <label>Coincidencia sugerida
          <select data-voice-match="${suggestion.id}">
            ${suggestion.matches.length ? suggestion.matches.map((match) => `<option value="${match.libraryIndex}" ${String(match.libraryIndex) === String(suggestion.selectedLibraryIndex) ? "selected" : ""}>${escapeHtml(match.name)} - ${Math.round(match.score * 100)}%</option>`).join("") : `<option value="">Sin coincidencia clara</option>`}
          </select>
        </label>
        <div class="voice-suggestion-meta">
          <span>Categoría: ${escapeHtml(selected?.category || "Sin categoria")}</span>
          <span>Fuente: ${escapeHtml(selected?.source || "Dictado")}</span>
        </div>
        <div class="voice-dose-grid">
          <label>Series<input data-voice-field="${suggestion.id}:sets" inputmode="numeric" value="${escapeHtml(suggestion.sets)}"></label>
          <label>Reps<input data-voice-field="${suggestion.id}:reps" inputmode="numeric" value="${escapeHtml(suggestion.reps || "")}" placeholder="-"></label>
          <label>Seg<input data-voice-field="${suggestion.id}:seconds" inputmode="numeric" value="${escapeHtml(suggestion.seconds || "")}" placeholder="-"></label>
          <label>Kg<input data-voice-field="${suggestion.id}:load" inputmode="decimal" value="${escapeHtml(loadToInputValue(suggestion.load || ""))}" placeholder="Sin carga"></label>
        </div>
        <div class="button-row voice-suggestion-actions">
          <button class="primary-action" data-voice-use="${suggestion.id}" type="button">Usar sugerencia</button>
          <button class="secondary-action" data-voice-search="${suggestion.id}" type="button">Buscar manualmente</button>
          <button class="danger-action" data-voice-discard="${suggestion.id}" type="button">Descartar</button>
        </div>
      </article>
    `;
  }).join("");
...
function processVoiceRoutineText() {
  const input = $("#voice-routine-text");
  if (!input) return;
  routineVoiceSuggestions = parseRoutineVoiceText(input.value);
  renderVoiceRoutineSuggestions();
  if (!routineVoiceSuggestions.length) {
    setVoiceRoutineStatus("No pude detectar ejercicios completos. Probá: Sentadilla 3 series 12 repeticiones 10 kilos.", "warn");
    return;
  }
  setVoiceRoutineStatus(`${routineVoiceSuggestions.length} ejercicio${routineVoiceSuggestions.length === 1 ? "" : "s"} detectado${routineVoiceSuggestions.length === 1 ? "" : "s"}. Revisá y confirmá cada tarjeta.`, "ok");
}

function updateVoiceSuggestionField(suggestionId, field, value) {
  const suggestion = routineVoiceSuggestions.find((item) => item.id === suggestionId);
  if (!suggestion) return;
  if (field === "selectedLibraryIndex") {
    suggestion.selectedLibraryIndex = value;
  } else if (field === "load") {
    suggestion.load = formatKgLoad(value);
  } else {
    suggestion[field] = getVoiceNumber(value) || "";
  }
}

function buildExerciseFromVoiceSuggestion(suggestion) {
  const selected = suggestion.matches.find((match) => String(match.libraryIndex) === String(suggestion.selectedLibraryIndex)) || suggestion.matches[0];
  const name = selected?.name || suggestion.detectedName;
  const load = suggestion.load ? formatKgLoad(suggestion.load) : "Sin carga";
  return {
    id: uid("ex"),
    name,
    category: selected?.category || "",
    source: selected?.source || "Dictado por voz",
    sets: Number(suggestion.sets) || 1,
    reps: suggestion.seconds ? "" : (Number(suggestion.reps) || ""),
    seconds: suggestion.seconds ? Number(suggestion.seconds) || "" : "",
    load,
    prescription: buildPrescription(Number(suggestion.sets) || 1, suggestion.seconds ? "" : (Number(suggestion.reps) || ""), suggestion.seconds ? Number(suggestion.seconds) || "" : "", load),
    progress: 0
  };
}

function useVoiceRoutineSuggestion(suggestionId) {
  const suggestion = routineVoiceSuggestions.find((item) => item.id === suggestionId);
  if (!suggestion) return;
  const exercise = buildExerciseFromVoiceSuggestion(suggestion);
  if (!exercise.reps && !exercise.seconds) {
    setVoiceRoutineStatus("Faltan repeticiones o segundos en esa sugerencia. Corregí la tarjeta antes de agregar.", "warn");
    return;
  }
  exerciseDraft.push(exercise);
  editingExerciseIndex = null;
  renderExerciseDraft();
  routineVoiceSuggestions = routineVoiceSuggestions.filter((item) => item.id !== suggestionId);
  renderVoiceRoutineSuggestions();
  setVoiceRoutineStatus(`Ejercicio agregado: ${exercise.name}. Podés editarlo abajo antes de guardar.`, "ok");
}

function searchVoiceRoutineSuggestion(suggestionId) {
  const suggestion = routineVoiceSuggestions.find((item) => item.id === suggestionId);
  if (!suggestion) return;
  const selected = suggestion.matches.find((match) => String(match.libraryIndex) === String(suggestion.selectedLibraryIndex)) || suggestion.matches[0];
  $("#exercise-library-search").value = suggestion.detectedName;
  $("#exercise-name").value = selected?.name || suggestion.detectedName;
  $("#exercise-sets").value = suggestion.sets || "";
  $("#exercise-reps").value = suggestion.seconds ? "" : suggestion.reps || "";
  $("#exercise-seconds").value = suggestion.seconds || "";
  $("#exercise-load").value = loadToInputValue(suggestion.load || "");
  $("#exercise-name").dataset.libraryCategory = selected?.category || "";
  $("#exercise-name").dataset.librarySource = selected?.source || "";
  renderExerciseLibrary();
  $("#exercise-library-search").scrollIntoView({ behavior: "smooth", block: "center" });
  setVoiceRoutineStatus("Busqué el texto detectado en la biblioteca. Podés elegir manualmente o editar el ejercicio rápido.", "live");
}

function discardVoiceRoutineSuggestion(suggestionId) {
  routineVoiceSuggestions = routineVoiceSuggestions.filter((item) => item.id !== suggestionId);
  renderVoiceRoutineSuggestions();
  setVoiceRoutineStatus(routineVoiceSuggestions.length ? `${routineVoiceSuggestions.length} sugerencia${routineVoiceSuggestions.length === 1 ? "" : "s"} pendiente${routineVoiceSuggestions.length === 1 ? "" : "s"}.` : "No quedan sugerencias pendientes.", "");
}
...
function toggleRoutineVoiceRecognition() {
  ...
  routineSpeechRecognition.onend = () => {
    ...
    button.textContent = "Dictar rutina";
    if (input.value.trim()) {
      setVoiceRoutineStatus("Dictado listo. Revisá el texto y tocá Procesar texto.", "ok");
    }
  };
...
function bindEvents() {
  ...
  $("#voice-routine-apply")?.addEventListener("click", processVoiceRoutineText);
  ...
  if (!getSpeechRecognitionConstructor()) {
    ...
    setVoiceRoutineStatus("El dictado por voz no está disponible en este navegador. Podés escribir o pegar el texto de la rutina manualmente.", "warn");
  }
  ...
  document.body.addEventListener("click", (event) => {
    ...
    const voiceSearchButton = event.target.closest("[data-voice-search]");
    const voiceDiscardButton = event.target.closest("[data-voice-discard]");
    ...
    if (voiceUseButton) {
      useVoiceRoutineSuggestion(voiceUseButton.dataset.voiceUse);
    }
    if (voiceSearchButton) {
      searchVoiceRoutineSuggestion(voiceSearchButton.dataset.voiceSearch);
    }
    if (voiceDiscardButton) {
      discardVoiceRoutineSuggestion(voiceDiscardButton.dataset.voiceDiscard);
    }
  ...
  document.body.addEventListener("input", (event) => {
    ...
    const voiceField = event.target.closest("[data-voice-field]");
    if (input) {
    ...
    if (voiceField) {
      const [suggestionId, field] = voiceField.dataset.voiceField.split(":");
      updateVoiceSuggestionField(suggestionId, field, voiceField.value);
    }
  });

  document.body.addEventListener("change", (event) => {
    const matchSelect = event.target.closest("[data-voice-match]");
    if (!matchSelect) return;
    updateVoiceSuggestionField(matchSelect.dataset.voiceMatch, "selectedLibraryIndex", matchSelect.value);
    renderVoiceRoutineSuggestions();
  });
